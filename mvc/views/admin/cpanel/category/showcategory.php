...
<!--                        <td style=" font-size: 16px;" class=""><a class="btn btn-primary" href="--><?//=base?><!--admin/statuscategory&id=--><?//=$values["id"]?><!--&status=--><?//=$values["status"]?><!--&page=--><?//=$data['currentpage']?><!--">--><?//=$values["status"] ?><!--</a></td>-->
                        ...
                        <td>
                             ...
                             <a style="height: 35px" class="btn btn-danger submit" href="javascript:void(0)" onclick="del(<?=$values['id']?>,'<?=$values['name'] ?>','<?=base.'admin/deletecategory/'?>','danh mục ',<?=$data['currentpage']?>,<?=$stt?>)" >Xóa</a>
                        </td>
                    </tr>
                    <?php } ?>
                </tbody>
